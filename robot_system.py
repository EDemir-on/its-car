...
Handles motor control and command processing
"""
...
log = logging.getLogger("robot_system")
...
def initialize_robot(input_handler=None):
    """
    Initialize and start the robot control system
    
    Args:
        input_handler: Optional input handler instance to start
    """
    ...
    with _system_lock:
        ...
        # Start input handler if provided
        if input_handler:
            input_handler.set_command_queue(cmd_queue)
            input_handler.start()
...
def shutdown_robot():
    ...
    with _system_lock:
        ...
        if _robot_system['input_handler']:
